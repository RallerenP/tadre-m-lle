        ...
        stage("Deploy") {
            ...
            steps {
                sshagent(credentials: ['SSH_CREDENTIALS']) {
                    ...
                    sh "ssh -v -o StrictHostKeyChecking=no ubuntu@3.239.98.129 'sudo docker stop tadre || true && sudo docker rm tadre || true && sudo docker rmi tadre:latest || true && sudo docker load < /tadre/tadre.tar && sudo docker run --name tadre -p 8081:80 -v /tmp/:/datapath/ --env DB_URL=\${DB_URL} --env DB_USER=\${DB_USER} --env DB_PASS=\${DB_PASS} tadre:latest'"
                }
            }
        }
    }
